...
def get_input():
    global temp_input, humid_input, adjust_state
    if adjust_state:
        temp_input = validate_input(req_temp_entry.get())
        humid_input = validate_input(req_humid_entry.get())
    else:
        temp_input = 0
        humid_input = 0

def log_to_history(message):
    history_list.insert(tk.END, message)  # Thêm vào Listbox
    history_list.yview(tk.END)

def show_warning(title, message):
    messagebox.showerror(title, message)

def data_processing(t,h,gas, auto_state):
    global temp_input, humid_input, fan_state, light_state, dehumidifier_state
    if t >= 28 or gas > 300:
        danger_message = f"DANGER: Temp={t}°C, Gas={gas} (Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')})"
        arduino.write(b'DANGER\n')
        if t >= 28:
            messagebox.showerror("Temperature Warning", "SOS: Temperature Exceeded!")
        elif gas > 300:
            messagebox.showerror("Smoke/Gas Warning", "SOS: Gas or Smoke Level Exceeded!")
        fan_state = False
        light_state = False
        dehumidifier_state = False
        log_to_history(danger_message)
        ...
        if auto_state:
            if humid_input == 0 or not adjust_state:
                if h > 70:
                    arduino.write(b'HIGH_HUMID\n')
                    arduino.write(b'WARM_OFF\n')
            ...
            if temp_input == 0 or not adjust_state:
                if t < 28 and t > 25:
                    arduino.write(b'NOR_TEMP\n')
                    arduino.write(b'WARM_OFF\n')
            ...
            elif temp_input != 0 and adjust_state:
                if t > temp_input:
                    arduino.write(b'NOR_TEMP\n')
                    arduino.write(b'WARM_OFF\n')
                    arduino.write(b'FAN_ON\n')
            ...
            if gas <= 300:
                arduino.write(b'NOR_GAS\n')

def get_sensor_data():
    global fan_state, light_state, temperature, humidity, gas_smoke, auto_state
    try:
        if arduino.in_waiting > 0:
            ...
            if data.startswith("Humidity:"):
                ...
                update_humidity_bar(humidity)

            elif data.startswith("Temperature:"):
                temperature = float(data.split(":")[1].strip().replace("°C", ""))
                update_temperature_bar(temperature)

            elif data.startswith("Gas:"):
                gas_smoke = int(data.split(":")[1].strip())
                update_gas_bar(gas_smoke)

            data_processing(temperature, humidity, gas_smoke, auto_state)

    except Exception as e:
        print(f"Error When Reading Data: {e}")

    root.after(100, get_sensor_data)
...
temperature_canvas.create_text(193.4545, 10, text="28°C", font=("Arial", 10))
